...
export default function ImageDialog({ images, name }) {
  ...
  const displayedImages = showAllImages ? images : [images[0]];

  return (
    <Dialog.Root>
      <div className="grid  w-full grid-cols-1 sm:h-20">
        {displayedImages.map((image) => (
          <Dialog.Trigger key={image} asChild>
            <button type="button" onClick={() => handleClick(image)}>
              <img src={image} alt={name} className="h-48 w-full rounded object-cover" />
            </button>
          </Dialog.Trigger>
        ))}
      </div>
...
//     <Dialog.Root>
//       {images.length === 1 && (
//         <div className="grid aspect-square h-1/2 w-full grid-cols-1 sm:h-20">
//           <Dialog.Trigger asChild>
//             <button type="button" className="aspect-square h-full" onClick={() => setSelectedImage(images[0])}>
//               <img src={images[0]} alt={name} className="h-48 w-full rounded object-cover" />
//             </button>
//           </Dialog.Trigger>
//         </div>
//       )}
//       {images.length === 2 && (
//         <div className="grid aspect-square h-1/2 grid-cols-2 gap-[3px] sm:h-20">
//           {images.map((image) => (
//             <Dialog.Trigger key={image} asChild>
...
//             <Dialog.Trigger key={image} className={`${index === 0 ? 'row-span-2' : ''}`} asChild>
//               <button type="button" className="h-full w-full" onClick={() => setSelectedImage(image)}>
//                 <img src={image} alt={name} className="h-full w-full rounded object-cover" />
//               </button>
//             </Dialog.Trigger>
